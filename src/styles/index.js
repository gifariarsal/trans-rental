export const styles = {
  sectionContainer: 'w-full px-4 md:px-10 py-16 gap-12',
  pages: 'max-w-[1440px] mx-auto px-4 md:px-14 py-8',
  sectionHeader: 'text-center mx-auto mb-4 md:mb-10',
  sectionTitle: 'text-4xl lg:text-5xl text-primary font-bold',
  sectionSubtitle: 'text-lg lg:text-xl text-blackShade font-bold mb-2',
  chevronButton:
    'mb-[280px] bg-black hover:bg-blackShade border-[1.5px] border-primary rounded-full p-2 transition-all duration-300',
};
